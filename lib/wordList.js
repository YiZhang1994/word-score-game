...
var Word_List = (function () {
    ...
    /**
     * Use regex to check word list when there is "_" in the words
     * @param toCheck
     */
    function regexCheck(toCheck) {
        console.log("enter regexCheck");
        //console.log(wordBank);
        toCheck = toCheck.toLowerCase();
        var initialLetter = toCheck.charAt(0);
        var possibleWords = [];
        if (initialLetter != "_") {
            var regex = eval("/^" + toCheck.replace(/\_/g, "[a-z]") + "$/");
            for (var i in wordBank[initialLetter]) {
                if (regex.test(wordBank[initialLetter][i])) {
                    possibleWords.push(wordBank[initialLetter][i]);
                }
            }
        }
        if (initialLetter == "_") {
            for (var i = 0; i < 26; i++) {
                var initial = String.fromCharCode(97 + i);
                var newstr = initial + toCheck.slice(1);
                var regex = eval("/^" + newstr.replace(/\_/g, "[a-z]") + "$/")
                for (var j in wordBank[initial]) {
                    if (regex.test(wordBank[initial][j])) {
                        possibleWords.push(wordBank[initial][j]);
                    }
                }
            }
        }
    ...
    publicInt.isRegexMatch = regexCheck;
